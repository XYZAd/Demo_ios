...
//    [_tableView registerClass:[AdTableViewCell class] forCellReuseIdentifier:@"adcell"];
    ...
    for (int i = 0; i < 50; i ++) {
        ...
        if (i % 5 == 2) {
            ...
            XMAdImgTextParam *para = [XMAdImgTextParam new];
            para.position = kDemoImgText;
            //1280*720
            para.expectAdSize = CGSizeMake(1280, 720);
    ...
    if (model.feedType == XYZFeedType_Ad) {
        if (model.ad) {
            if (model.ad.materialMeta.imgTextRenderType == XMImgTextRenderTypeExpress) {
                return model.ad.materialMeta.expressView.frame.size.height;
            } else {
                CGFloat radio = model.ad.materialMeta.coverImage.imgWidth / CGRectGetWidth(UIScreen.mainScreen.bounds);
                return 30 + model.ad.materialMeta.coverImage.imgHeight / MAX(1, radio);
            }
        }
        return CGFLOAT_MIN;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.feedType == XYZFeedType_Ad) {
        if (model.ad.materialMeta.imageMode == XMFeedAdMode_Text) {
            identifier = @"textcell";
