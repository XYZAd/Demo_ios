...
- (void)initialAd {
    [_imageView yy_setImageWithURL:[NSURL URLWithString:self.nativeAd.materialMeta.coverImage.imgURL] options:0];
    ...
    [titleLabel setText:self.nativeAd.materialMeta.adTitle ? : self.nativeAd.materialMeta.adSubTitle];
    ...
    adStateLabel.text = self.nativeAd.materialMeta.isAppAd ? @"立即下载" : @"查看详情";
    ...
    if (self.nativeAd.materialMeta.logoImage.image) {
        logoImageView.image = self.nativeAd.materialMeta.logoImage.image;
        ...
        [logoImageView yy_setImageWithURL:[NSURL URLWithString:self.nativeAd.materialMeta.logoImage.imageURL] placeholder:nil options:0 completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
            [logoImageView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    CGFloat radio = self.nativeAd.materialMeta.coverImage.imgWidth / width;
    CGFloat imageHeight = self.nativeAd.materialMeta.coverImage.imgHeight / radio;
    [self.imageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(@0);
        make.size.mas_equalTo(CGSizeMake(width, imageHeight));
    }];
    
    if (self.nativeAd.materialMeta.imageMode == XMFeedADMode_VideoImage) {
        [self.imageView insertSubview:self.nativeAd.materialMeta.videoView atIndex:0];
        [self.nativeAd.materialMeta.videoView mas_makeConstraints:^(MASConstraintMaker *make) {
